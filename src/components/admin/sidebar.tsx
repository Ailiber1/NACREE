...
const navItems = [
  ...
  { href: "/admin/bookings", label: "予約一覧", icon: "bookings" },
  { href: "/admin/customers", label: "顧客管理", icon: "customers" },
  { href: "/admin/menus", label: "メニュー管理", icon: "menus" },
  { href: "/admin/staff", label: "スタッフ管理", icon: "staff" },
  { href: "/admin/reports", label: "売上レポート", icon: "reports" },
...
export function AdminSidebar() {
  ...
  return (
    <>
      ...
      {/* サイドバー */}
      <aside
        className={`fixed top-0 left-0 z-40 h-screen w-64 flex flex-col transition-transform duration-200 lg:translate-x-0 ${
      ...
      >
        ...
        <div className="px-6 py-6 border-b" style={{ borderColor: "rgba(255,255,255,0.08)" }}>
          <Link
            ...
            style={{ fontFamily: "'Noto Serif JP', serif", color: "#B8956A" }}
            onClick={() => setMobileOpen(false)}
          >
            NACR&Eacute;E
          </Link>
        ...
        <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
          {navItems.map((item) => (
            <Link
              ...
              style={{
                backgroundColor: isActive(item.href) ? "rgba(184,149,106,0.12)" : "transparent",
                color: isActive(item.href) ? "#B8956A" : "rgba(255,255,255,0.6)",
              }}
            >
